...
async function fetchTeamJobs(team) {
  ...
  if (!teamJobs[team.teamId]) {
    try {
      const res = await axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/team/${team.teamId}/job`)
  ...
  if (!teamRanks[team.teamId]) {
    try {
      ...
      const jobs = (await axios.get(
        `https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/team/${team.teamId}/job`
      )).data
      ...
      const ranksList = await Promise.all(
        jobs.map(async (j) => {
          const [ranks, positions] = await Promise.all([
            axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/job/${j.jobId}/ranks`).then(r => r.data),
            axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/job/${j.jobId}/positions`).then(r => r.data)
          ])
