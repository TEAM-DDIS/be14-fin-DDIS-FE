...
<script setup>
  ...
  onMounted(async () => {
    try {
      ...
      const res = await fetch('https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/attendance/leave/history/used/all', {
        method: 'GET',
        headers: {
  ...
  function downloadCSV() {
    ...
    link.href = url
    link.setAttribute('download', '연차사용내역.csv')
