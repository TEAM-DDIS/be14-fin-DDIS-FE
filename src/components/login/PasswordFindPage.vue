...
watch(employeeId, async id => {
  ...
  try {
    const r = await fetch(`http://localhost:8000${BASE}/mfa/exists?empId=${id}`)
...
async function requestCode() {
  clearMsg()
  try {
    const res = await fetch(`http://localhost:8000${BASE}/request`, {
      method: 'POST',
...
async function verifyCode() {
  clearMsg()
  try {
    const res = await fetch(`http://localhost:8000${BASE}/verify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function registerOtp() {
  ...
  try {
    const res = await fetch(
      `http://localhost:8000${BASE}/mfa/qr?empId=${employeeId.value}`
    )
...
async function verifyOtp() {
  clearMsg()
  try {
    const res = await fetch(`http://localhost:8000${BASE}/mfa/verify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function resetPassword() {
  ...
  try {
    const res = await fetch(`http://localhost:8000${BASE}/change-password`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json'
       },
      body: JSON.stringify({
