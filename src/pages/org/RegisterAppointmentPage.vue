...
onMounted(async () => {
  try {
    const resp = await axios.get('https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/structure/hierarchy')
...
async function loadEmployeeInfo() {
  ...
  if (!emp) {
    try {
      const res = await axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/employee/${id}`)
      emp = res.data
...
watch(() => form.org.headId, async headId => {
  ...
  const r = await axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/structure/heads/${headId}/departments`)
...
watch(() => form.org.departmentId, async deptId => {
  ...
  const r = await axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/structure/departments/${deptId}`)
...
watch(() => form.org.teamId, async teamId => {
  ...
  const r = await axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/team/${teamId}/job`)
...
watch(() => form.org.jobId, async jobId => {
  ...
  if (jobId) {
    const [posRes, rankRes] = await Promise.all([
      axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/job/${jobId}/positions`),
      axios.get(`https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/introduction/job/${jobId}/ranks`)
    ])
...
async function submit() {
  ...
  try {
    await axios.post(
      'https://ddis-be-alb-1219702514.ap-northeast-2.elb.amazonaws.com/appointment/create',
      payload,
